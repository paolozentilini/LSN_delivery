...
double Statistics :: chi_squared(vector<int> counter, double expected_value){
	double appo,chi_squared=0;
	for (unsigned int i = 0; i < counter.size(); i++){
		appo = counter.at(i);
		chi_squared += (( appo - expected_value )*( appo - expected_value))/expected_value;			//chi squared calculation
	}
	return chi_squared;
}

void Statistics :: chi_squared_test(unsigned int n, unsigned int throws, const char* results_file){
	vector<double> chi_squared_measurements;
	vector<int> counter;
	double expected_value = throws/n;
	double appo;
	ofstream outfile;
	int nchi = 1e4;
	outfile.open(results_file);
	if(outfile.is_open()){
		// how many chi2 i want
		for(int ichi=0; ichi<nchi; ichi++){
			// back counter to zero
			counter.assign(n,0);
			// throws for each chi2 evaluation
			for(unsigned int k=0; k<throws; k++){
				appo = _random -> get_uniform();
				for(unsigned int i=0; i<n; i++){
					if ( (double(i)/double(n) < appo) && (appo < double(i+1)/double(n))){ counter[i] += 1; }
				}
			}
			if(ichi%100==0) cout << ichi << endl;
			// save my ichi-th chi2
	...
	if (outfile.is_open()){
		for(unsigned int i=0; i<_n_blocks; i++){
			...
			for(unsigned int j=0; j<_n_per_blocks; j++){
				single_measure += _experiment -> single_measurement(_random -> get_uniform());
			}
