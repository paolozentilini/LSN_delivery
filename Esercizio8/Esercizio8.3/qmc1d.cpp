...
int main(){
  readInput();
	initialize();
  /* at this time, every variable you see, such for instance "equilibration",  has been either acquired
  from "input.dat" by the readInput() function or opportunely initialized by the initialize() function. */
  //EQUILIBRAZIONE
	for(int i=0; i<equilibration; i++){
    // only a PIGS polymer(T=0K) has a start and an end.
		if(PIGS){ //Se sono a temperatura 0 vado a muovere a sx e a dx il polimero aperto
			brownianMotion(LEFT);
  ...
  //SIMULAZIONE
	for(int b=0;b<blocks;b++){
		for(int i=0;i<MCSTEPS;i++){ //MCSTEPS = numero di MC step per blocco
      //Se sono a T=0K muovo le code, altrimenti solo traslazione rigida e ricostruzione della parte interna del polimero
			if(PIGS){
				brownianMotion(LEFT);
				brownianMotion(RIGHT);
			}
			...
			for(int j=0;j<brownianBridgeAttempts;j++)
				brownianBridge();
			upgradeAverages(); //Upgrade delle medie di blocco
		}
		if(b%5==0) cout<<"Completed block: "<<b+1<<"/"<<blocks<<endl;
		endBlock();
	}
	consoleOutput();
  //Stampa dei risultati ottenuti e deallocazione della memoria dinamica
...
void initialize(){
	...
	if(PIGS)
		dtau = imaginaryTimePropagation/(timeslices-1); //Se siamo a T=0K usiamo il tempo immaginario
	else
		dtau = hbar/(boltzmann*temperature*timeslices); //Se T finito allora dtau = beta/M
  ...
  totalBM=0;
	...
	for(int i=0;i<timeslices;i++) {                      //Ho dei vettori poiché ho M particelle classiche nel polimero
...
double variationalWaveFunction(double v){
  ...
  if(wavefunc==0){
    ...
    double sigma = 0.61, mu = 0.8;
    double psi = exp(-(v-mu)*(v-mu)/(2.*sigma*sigma)) +  exp(-(v+mu)*(v+mu)/(2.*sigma*sigma));
    return psi;
  }
}
...
double variationalWaveFunction_second(double v){
  if(wavefunc==0){
    ...
    double sigma = 0.61, mu = 0.8;
    double s_2 = sigma*sigma;
  	return exp(-(v+mu)*(v+mu)/(2.*s_2) ) * ( exp( 2.*mu*v/s_2 ) * ( (v-mu)*(v-mu)-s_2 ) + (v+mu)*(v+mu) -s_2)/(s_2*s_2);
  }
	//return v*v*exp(-0.5*v*v) - exp(-0.5*v*v);
}

//Traslation() prende le coordinate del polimero (immagazzinate in positions[]) e le trasla rigidamente di una quantità delta
...
void translation(){
    ...
    //potential_density_matrix calcola solo il contributo potenziale.
		newcorr = potential_density_matrix(positions[i]+delta,positions[inext]+delta);    //Probabilità nel punto nuovo
		oldcorr = potential_density_matrix(positions[i],positions[inext]);                //Probabilità nel punto vecchio
	...
	if(PIGS) //Se T=0K nell'accettazione devo contare i contributo delle f. d'onda di trial agli estremi della catena!
...
void finalizePotentialEstimator(){
	ofstream out;
  if(wavefunc==0){
    out.open("../../Esercizio8/Risultati/potential_const_wf_tau"+to_string(int(imaginaryTimePropagation))+".dat");
  }else{
    out.open("../../Esercizio8/Risultati/potential_not_const_wf_tau"+to_string(int(imaginaryTimePropagation))+".dat");
  }
	for(int i=0;i<timeslices;i++){
...
void finalizeKineticEstimator(){
	ofstream out;
  if(wavefunc==0){
    out.open("../../Esercizio8/Risultati/kinetic_const_wf_tau"+to_string(int(imaginaryTimePropagation))+".dat");
  }else{
    out.open("../../Esercizio8/Risultati/kinetic_not_const_wf_tau"+to_string(int(imaginaryTimePropagation))+".dat");
  }
	for(int i=0;i<timeslices;i++){
...
void finalizeHistogram(){
	ofstream out;
  if(wavefunc==0){
    out.open("../../Esercizio8/Risultati/probability_const_wf_tau"+to_string(int(imaginaryTimePropagation))+".dat");
  }else{
    out.open("../../Esercizio8/Risultati/probability_not_const_wf_PIMC_temperature"+to_string(int(temperature))+".dat");
  }
...
void readInput(){
  ...
  input_file >> string_away >> wavefunc;
	input_file.close();
	delete [] string_away;
}
