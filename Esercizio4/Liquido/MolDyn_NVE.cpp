...
void Equilibration(){
  ...
  for(int istep=1; istep<=n_equilibration; ++istep){
      ...
      if(istep%10) Measure_Equilibration();
  }
  for(int istep=1; istep<=5*n_equilibration; ++istep){
    Move();
    if(istep%10) Measure_Equilibration();
  }
...
void Measure_Equilibration(){ //Properties measurement
  ...
  Epot.open("../../Esercizio4/Risultati/liquid_equilibration_epot.dat",ios::app);
  Ekin.open("../../Esercizio4/Risultati/liquid_equilibration_ekin.dat",ios::app);
  Temp.open("../../Esercizio4/Risultati/liquid_equilibration_temp.dat",ios::app);
  Etot.open("../../Esercizio4/Risultati/liquid_equilibration_etot.dat",ios::app);
...
void Averages(int iblk){
  ...
  Epot.open("../../Esercizio4/Risultati/liquid_epot.dat",ios::app);
  Ekin.open("../../Esercizio4/Risultati/liquid_ekin.dat",ios::app);
  Temp.open("../../Esercizio4/Risultati/liquid_temp.dat",ios::app);
  Etot.open("../../Esercizio4/Risultati/liquid_etot.dat",ios::app);

  stima_pot = blk_av[iv]/blk_norm; //Potential energy
  glob_av[iv] += stima_pot;
  glob_av2[iv] += stima_pot*stima_pot;
  err_pot=Error(glob_av[iv],glob_av2[iv],iblk);
  Epot << glob_av[iv]/(double)iblk << setw(wd) << err_pot << setw(wd) << endl;
  ...
  Ekin << glob_av[ik]/(double)iblk << setw(wd) << err_kin << setw(wd) << endl;

  stima_etot = blk_av[ie]/blk_norm; //Total energy
  glob_av[ie] += stima_etot;
  glob_av2[ie] += stima_etot*stima_etot;
  err_etot=Error(glob_av[ie],glob_av2[ie],iblk);
  Etot << glob_av[ie]/(double)iblk << setw(wd) << err_etot << setw(wd) << endl;

  stima_temp = blk_av[it]/blk_norm; //Temperature
  glob_av[it] += stima_temp;
  glob_av2[it] += stima_temp*stima_temp;
  err_temp=Error(glob_av[it],glob_av2[it],iblk);
  Temp << glob_av[it]/(double)iblk << setw(wd) << err_temp << setw(wd) << endl;
